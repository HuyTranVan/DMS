...
public class StatisticalActivity extends BaseActivity implements  View.OnClickListener {
                ...
                if (listUser.size()>1){
                    ...
                    CustomBottomDialog.choiceList("Chọn nhân viên", users, new CustomBottomDialog.StringListener() {
                        ...
                        public void onResponse(String content) {
                            ...
                            Util.cashFragment.reloadData(convertToListPayment(tvEmployeeName.getText().toString().trim(),InitialBillHavePayment , getStartDay(currentDate) , getEndDay(currentDate)));
                            Util.checkinFragment.reloadData(convertToListCheckin(tvEmployeeName.getText().toString().trim(),
                                    InitialCheckin,
                                    getStartDay(currentDate),
                                    getEndDay(currentDate)) );
    ...
    private void loadInitialBills(){
        String billParam = String.format(Api_link.BILL_RANGE_PARAM, getStartDay(currentDate), getEndDay(currentDate));

        CustomerConnect.ListBill(billParam, new CallbackJSONArray() {
            @Override
    ...
    private void loadListPayment(){
        ...
        CustomerConnect.ListBillHavePayment(billParam, new CallbackJSONArray() {
            @Override
            public void onResponse(JSONArray result) {
    ...
    private void loadListCheckin(){
        String param = String.format(Api_link.CUSTOMER_CHECKIN_RANGE_PARAM, getStartDay(currentDate), getEndDay(currentDate));
    ...
    private long getStartDay(String currentdate){
        long date = 0;
    ...
    private long getEndDay(String currentdate){
        long date = 0;
