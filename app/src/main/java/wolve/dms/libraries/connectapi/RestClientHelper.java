...
public class RestClientHelper {
    ...
    public void postString(@NonNull String serviceUrl, @NonNull ArrayMap<String, Object> params, @NonNull RestClientListener restClientListener) {
        postString(serviceUrl, null, params, restClientListener);
    }

    public void postString(@NonNull String serviceUrl, ArrayMap<String, String> headers, @NonNull ArrayMap<String, Object> params, @NonNull RestClientListener restClientListener) {
        final Request.Builder builder = new Request.Builder();
        if (headers != null)
            addHeaders(builder, headers);
        StringBuffer urls = new StringBuffer();
        ...
        urls.append(serviceUrl);
        builder.url(urls.toString());
        builder.post(generateRequestBodyString(params));
        execute(builder, restClientListener);
    }

    public void postNonStringRequest(@NonNull String serviceUrl, ArrayMap<String, String> headers, @NonNull ArrayMap<String, Object> params, @NonNull RestClientListener restClientListener) {
        final Request.Builder builder = new Request.Builder();
        if (headers != null)
            addHeaders(builder, headers);
        StringBuffer urls = new StringBuffer();
            ...
            public void run() {
                ...
                try {
                    ...
                    handler.post(new Runnable() {
                        @Override
                        public void run() {
                            restClientListener.onError("Không có kết nối.");
                        }
                    });
                }
            }
    ...
    private RequestBody generateRequestBody(ArrayMap<String, Object> params) {
        ...
        Log.d(Constants.DMS_LOGS, jsonObj.toString());
        RequestBody requestBody = RequestBody.create(MediaType.parse("application/json; charset=utf-8"), String.valueOf(jsonObj));
        return requestBody;
    }

    private RequestBody generateRequestBodyString(ArrayMap<String, Object> params) {
        final JSONObject jsonObj = new JSONObject();
        if (params != null) {
            for (String key : params.keySet()) {
                try {
    ...
    private RequestBody generateMultipartBody(ArrayMap<String, Object> params, ArrayMap<String, File> files) {
        ...
        if (files != null) {
            for (String key : files.keySet()) {
                builder.addFormDataPart(key, key, RequestBody.create(MediaType.parse("image/logo_signup.png"), files.get(key)));
            }
        }
